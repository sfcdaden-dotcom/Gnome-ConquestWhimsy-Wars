...
| 14 | Garden upgrades & per-player supply | ⬜ | Design finalized with the designer 2026-07-28: 2-Wish upgrade action, tile-sticky (capturable) upgrades per garden type, per-player tile supply (4/type) replacing the shared bank, wild preset tiles. Spec: RULES.md ("Garden Upgrades" + per-garden **Upgraded** entries); rationale + implementation map: GARDEN_UPGRADES.md |
